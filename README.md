# .NET Continuous Learning

On a personal level, continuous learning is about the constant expansion of skills and skill-sets through learning and increasing knowledge.

## C#

C# is an elegant and type-safe object-oriented language that enables developers to build a variety of secure and robust applications that run on the .NET Framework. 

### Study Logs

Lesson: [C# Tutorial from Derek Banas Youtube Channel](https://www.youtube.com/watch?v=0p0JLFZj2C8&list=PLGLfVvz_LVvRX6xK1oi0reKci6ignjdSa) | May 8, 2018 to May 18, 2018.
    related repos : [tutorial exercises](https://github.com/diogowatson/continuous-learning-dotnet/tree/master/csharp-tutorial-erik-banas)
                    [wpf exercise 1](https://github.com/diogowatson/continuous-learning-dotnet/tree/master/csharp-tutorial-erik-banas-wpf1)
                    [wpf exercise 2](https://github.com/diogowatson/continuous-learning-dotnet/tree/master/csharp-tutorial-erik-banas-wpf2)
                    [Paint programm](https://github.com/diogowatson/continuous-learning-dotnet/tree/master/csharp-tutorial-erik-banas-paint-program)
                    [Paint app](https://github.com/diogowatson/continuous-learning-dotnet/tree/master/csharp-tutorial-erik-banas-paint-app)
                    
Lesson : [Microsoft Virtual Academy Test Driven Development Course](https://mva.microsoft.com/en-us/training-courses/testdriven-development-16458?l=lU8LSHwXC_806218965) | Completed

Lesson : [Udemy course : The Complete ASP.NET MVC 5 Course](https://www.udemy.com/the-complete-aspnet-mvc-5-course/learn/v4/content) | In progress
